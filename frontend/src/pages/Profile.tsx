...
import { ArrowLeft, User as UserIcon, Layout, Award, Image as ImageIcon, MessageCircle, Camera, ExternalLink, MapPin, CheckCircle, Tags } from 'lucide-react';
...
import { PROFILE_INTERESTS_TAB_LABEL } from '../content/userProfileGlossary';

const ProfilePersonalTab = lazy(() =>
    import('../components/profile/ProfilePersonalTab').then((m) => ({ default: m.ProfilePersonalTab })),
);
const ProfileInterestsTab = lazy(() =>
    import('../components/profile/ProfileInterestsTab').then((m) => ({ default: m.ProfileInterestsTab })),
);
const ProfileCoachTab = lazy(() =>
    import('../components/profile/ProfileCoachTab').then((m) => ({ default: m.ProfileCoachTab })),
);
const ProfileExperienceTab = lazy(() =>
    import('../components/profile/ProfileExperienceTab').then((m) => ({ default: m.ProfileExperienceTab })),
);
const ProfileGalleryTab = lazy(() =>
    import('../components/profile/ProfileGalleryTab').then((m) => ({ default: m.ProfileGalleryTab })),
);
const ProfileFAQTab = lazy(() =>
    import('../components/profile/ProfileFAQTab').then((m) => ({ default: m.ProfileFAQTab })),
);
const ProfileProgressTab = lazy(() =>
    import('../components/profile/ProfileProgressTab').then((m) => ({ default: m.ProfileProgressTab })),
);

function ProfileTabFallback() {
    return (
        <div
            className="flex min-h-[240px] items-center justify-center rounded-xl border border-gray-100 bg-gray-50/90"
            aria-busy="true"
            aria-live="polite"
        >
            <div
                                ...
                                <div className="flex items-center gap-3 pb-2 w-full md:w-auto">
                                    ...
                                    {(user.user_type === 'trainer' || user.user_type === 'athlete') && myProfile?.is_profile_public && (
                                        <a
                                            ...
                                            className="p-3 bg-white border border-gray-200 text-gray-600 rounded-lg hover:border-black hover:text-black transition-all shadow-sm group"
                ...
                <div className="bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-16 z-30">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center">
                        <div className="flex items-center gap-4 overflow-x-auto no-scrollbar after:content-[''] after:block after:w-4 after:shrink-0">
                            ...
                            <div className="h-6 w-px bg-gray-100 shrink-0"></div>
                            {tabs.map((tab) => (
                                <button
                                    key={tab.key}
                        ...
                        <div className="w-full lg:w-[320px] shrink-0 bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden sticky top-[120px]">
                            ...
                            <div className="h-32 bg-gray-900 relative">
                                {myProfile?.cover_image_url ? (
                                    ...
                                    <div className="w-full h-full flex flex-col items-center justify-center text-gray-500 bg-gradient-to-br from-gray-800 to-gray-900 gap-1">
                                    ...
                                    {tabs.map((tab) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <tab.icon className={`w-5 h-5 ${activeTab === tab.key ? 'text-white' : 'text-gray-500 group-hover:text-black'}`} />
                                                {tab.label}
                                            </div>
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                >
                                    <Suspense fallback={<ProfileTabFallback />}>
                                        {activeTab === 'personal' && (
                                            <ProfilePersonalTab user={user} onUpdate={() => dispatch(fetchMyProfile())} />
                                        )}
                                        {activeTab === 'interests' && <ProfileInterestsTab />}
                                        ...
                                        {activeTab === 'experience' && (user.user_type === 'trainer' || user.user_type === 'athlete') && (
                                            <ProfileExperienceTab />
                                        )}
                                        {activeTab === 'gallery' && (user.user_type === 'trainer' || user.user_type === 'athlete') && (
                                            <ProfileGalleryTab />
                                        )}
