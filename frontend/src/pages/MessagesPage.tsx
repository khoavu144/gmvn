...
export default function MessagesPage() {
    ...
    return (
        <div className="h-[calc(100vh-140px)] bg-white flex overflow-hidden relative">
            ...
            {/* Sidebar */}
            <div className={`w-full md:w-80 bg-[color:var(--mk-paper)] border-r border-[color:var(--mk-line)] flex-col shrink-0 ${activePartner ? 'hidden md:flex' : 'flex'}`}>
                <div className="p-4 border-b border-[color:var(--mk-line)] bg-white">
                ...
                <div className="flex-1 overflow-y-auto">
                    {error && conversations.length === 0 ? (
                        ...
                        <div className="p-8 text-center mt-10">
                            ...
                            <h3 className="font-bold text-[color:var(--mk-text)] mb-1">Hộp thư trống</h3>
                            <p className="text-xs text-[color:var(--mk-muted)] mb-6">
                        ...
                        <div className="divide-y divide-gray-100">
                            {conversations.map(conv => (
                                <button
                                    ...
                                    className={`w-full text-left p-4 hover:bg-[color:var(--mk-paper)] transition-colors ${activePartner === conv.partner_id ? 'bg-white border-l-2 border-black -ml-[2px]' : ''}`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-full border border-[color:var(--mk-line)] bg-[color:var(--mk-paper)] flex items-center justify-center text-sm font-bold text-[color:var(--mk-muted)] flex-shrink-0">
                                            {conv.partner?.full_name?.charAt(0).toUpperCase() || '?'}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between mb-0.5">
                                                <span className={`text-sm truncate ${conv.unread_count > 0 ? 'font-bold text-black' : 'font-medium text-[color:var(--mk-text)]'}`}>
                                            ...
                                            {conv.last_message && (
                                                <p className={`text-xs truncate ${conv.unread_count > 0 ? 'text-black font-medium' : 'text-[color:var(--mk-muted)]'}`}>
            ...
            <div className={`flex-1 flex-col bg-white w-full h-full absolute inset-0 md:static ${!activePartner ? 'hidden md:flex' : 'flex'}`}>
                {activePartner ? (
                    <>
                        <div className="bg-white border-b border-[color:var(--mk-line)] p-4 flex items-center gap-3">
                            <button 
                                onClick={() => setActivePartner(null)} 
                                className="md:hidden p-1 -ml-2 text-[color:var(--mk-muted)] hover:text-black hover:bg-[color:var(--mk-paper)] rounded-full transition-colors"
                            >
                                <ChevronLeft className="w-6 h-6" />
                            </button>
                            <div className="w-8 h-8 rounded-full border border-[color:var(--mk-line)] bg-[color:var(--mk-paper)] flex items-center justify-center text-xs font-bold text-[color:var(--mk-text-soft)]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-[color:var(--mk-paper)]/50">
                            {loading ? (
                                ...
                                <div className="h-full flex items-center justify-center">
                                    <div className="text-center text-[color:var(--mk-muted)] text-sm border border-dashed border-[color:var(--mk-line)] p-6 rounded-xs">
                                        Hãy gửi lời chào!
                                    </div>
                                </div>
                            ) : (
                        ...
                        <div className="bg-white border-t border-[color:var(--mk-line)] p-4">
                            <div className="max-w-4xl mx-auto flex gap-3 relative">
                                <input
                                    ...
                                    className="form-input rounded-full px-5 py-3 pr-24 bg-[color:var(--mk-paper)] border-[color:var(--mk-line)] focus:bg-white"
                                />
                                <button
                                    onClick={sendMessage}
                                    disabled={!newMessage.trim()}
                                    className="absolute right-1 top-1 bottom-1 px-4 bg-black text-white hover:bg-gray-800 disabled:bg-[color:var(--mk-paper-strong)] rounded-full text-sm font-semibold transition-colors"
                                >
                                    Gửi
                                </button>
                            </div>
                    ...
                    <div className="flex-1 flex items-center justify-center text-[color:var(--mk-muted)] bg-[color:var(--mk-paper)]">
