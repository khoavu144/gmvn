            ...
            <div className="flex gap-2 mb-10 border-b border-gray-200 pb-0 overflow-x-auto">
                {['Tổng quan', 'Dành cho học viên', 'Dành cho HLV'].map((tab, i) => (
                    <a
                        key={tab}
                        href={`#section-${i}`}
                        className="px-4 py-2.5 text-sm font-medium text-gray-600 hover:text-black border-b-2 border-transparent hover:border-gray-300 whitespace-nowrap transition-colors"
                    >
                        {tab}
                    </a>
                ))}
