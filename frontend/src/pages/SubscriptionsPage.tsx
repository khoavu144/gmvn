        ...
        <div className="page-shell">
            ...
            <div className="page-container max-w-4xl py-12">
                ...
                {loading ? (
                    ...
                    <div className="card text-center py-12 bg-white border border-gray-200 shadow-sm rounded-2xl">
                        <CreditCard className="w-12 h-12 mx-auto text-gray-300 mb-4" />
                        <h3 className="text-xl font-bold mb-2 tracking-tight">Bạn chưa đăng ký gói tập nào</h3>
                        <p className="text-gray-500 mb-6 text-sm">Khám phá các Coach chuyên nghiệp và tham gia chuyên đề để bắt đầu lộ trình.</p>
                        <Link to="/coaches" className="btn-primary inline-block uppercase tracking-widest text-xs">Tìm Coach ngay</Link>
                    </div>
                ) : (
                    <div className="space-y-6">
                        {subscriptions.map((sub: any) => (
                            <div key={sub.id} className="card bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col sm:flex-row gap-6 justify-between items-start sm:items-center transition hover:shadow-md">
                                <div className="space-y-2">
                                    <div className="flex items-center gap-3">
                                        ...
                                        {sub.status === 'active' ? (
                                            ...
                                            <span className="flex items-center gap-1 text-xs font-bold px-2 py-1 bg-gray-50 border border-gray-200 text-gray-700 rounded-md uppercase tracking-wider">
                                    ...
                                    {sub.next_billing_date && sub.status === 'active' && (
                                        <p className="text-xs text-gray-400 mt-1 font-mono bg-gray-50 p-1.5 rounded inline-block">Gia hạn tiếp theo: {new Date(sub.next_billing_date).toLocaleDateString('vi-VN')}</p>
                                    )}
                                </div>
                                <div className="w-full sm:w-auto flex sm:flex-col gap-3">
                                    <Link to={`/programs/${sub.program_id}`} className="flex-1 text-center py-2 px-6 shadow-sm border border-gray-300 rounded-md font-bold text-xs uppercase tracking-widest text-black hover:bg-gray-50 transition-colors whitespace-nowrap">
                                        Xem lớp học
                                    </Link>
