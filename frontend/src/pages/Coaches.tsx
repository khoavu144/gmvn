        ...
        >
            <div className="flex items-center gap-4 mb-4">
                <div className="shrink-0">
                    {trainer.avatar_url ? (
                        <img className="w-16 h-16 rounded-xs object-cover border border-[color:var(--mk-line)] transition-all" src={trainer.avatar_url} alt={trainer.full_name} fetchPriority="low" loading="lazy" decoding="async" />
                    ) : (
                        <div className="w-16 h-16 bg-[color:var(--mk-paper)] border border-[color:var(--mk-line)] rounded-xs flex items-center justify-center text-[color:var(--mk-muted)] text-xl font-bold uppercase">
                ...
                <div className="min-w-0">
                    ...
                    <p className="text-sm font-medium text-[color:var(--mk-text-soft)] mt-0.5">
            ...
            <p className="text-sm text-[color:var(--mk-text-soft)] line-clamp-3 flex-1 mb-4">
            ...
            {trainer.specialties && trainer.specialties.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                    {trainer.specialties.slice(0, 3).map((spec: string) => (
                        <span key={spec} className="inline-flex py-0.5 px-2 bg-[color:var(--mk-paper)] border border-[color:var(--mk-line)] text-[color:var(--mk-text-soft)] text-xs font-medium rounded-xs">
                            {spec}
                        </span>
                    ))}
                    {trainer.specialties.length > 3 && (
                        <span className="inline-flex py-0.5 px-2 bg-[color:var(--mk-paper)] border border-[color:var(--mk-line)] text-[color:var(--mk-muted)] text-xs font-medium rounded-xs">
                            +{trainer.specialties.length - 3}
                        </span>
                    )}
                </div>
            ...
            <div className="pt-3 border-t border-[color:var(--mk-line)] flex justify-between items-center text-sm font-medium text-black">
                Xem hồ sơ chi tiết
    ...
    return (
        <main className="page-shell">
            <Helmet>
                ...
                <link rel="canonical" href={currentType === 'athlete' ? 'https://gymerviet.com/coaches?type=athlete' : 'https://gymerviet.com/coaches'} />
                ...
                <meta property="og:url" content={currentType === 'athlete' ? 'https://gymerviet.com/coaches?type=athlete' : 'https://gymerviet.com/coaches'} />
            ...
            <div className="page-container">
                <section className="page-header relative">
                    {/* Tab Switcher */}
                    <div className="flex bg-[color:var(--mk-paper)] p-1 rounded-full w-max mx-auto lg:mx-0 mb-6 border border-[color:var(--mk-line)]">
                        <button
                            onClick={() => { setSearchParams({}); setPage(1); setSearch(''); setSpecialty(''); setPriceIdx(0); setSort('newest'); }}
                            className={`px-5 py-2 rounded-full text-sm font-bold transition-colors ${currentType === 'trainer' ? 'bg-white shadow-sm text-black' : 'text-[color:var(--mk-muted)] hover:text-black'}`}
                        >
                            Coach
                        </button>
                        <button
                            onClick={() => { setSearchParams({ type: 'athlete' }); setPage(1); setSearch(''); setSpecialty(''); setPriceIdx(0); setSort('newest'); }}
                            className={`px-5 py-2 rounded-full text-sm font-bold transition-colors ${currentType === 'athlete' ? 'bg-white shadow-sm text-black' : 'text-[color:var(--mk-muted)] hover:text-black'}`}
                        >
                            Vận động viên
                        </button>
                    </div>
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between -mt-2">
                        ...
                        {!isLoading && !isError && (
                            <div className="text-sm text-[color:var(--mk-muted)] font-medium">
                ...
                <div className="flex gap-3 flex-wrap items-center">
                    <div className="relative flex-1 min-w-[200px] max-w-xl">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg className="w-4 h-4 text-[color:var(--mk-muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                        </div>
                        <input
                            type="text"
                    ...
                    <button
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg border font-semibold text-sm transition-colors ${showFilters || hasActiveFilters ? 'border-black bg-black text-white' : 'border-[color:var(--mk-line)] text-[color:var(--mk-text-soft)] hover:border-black'}`}
                ...
                {/* ── Filter Panel ── */}
                {showFilters && (
                    <div className="mt-4 p-5 border border-[color:var(--mk-line)] rounded-sm bg-[color:var(--mk-paper)] space-y-5">
                        {/* Specialty */}
                        <div>
                            <label className="text-xs font-bold text-[color:var(--mk-muted)] uppercase tracking-wider block mb-2">Chuyên môn</label>
                        ...
                        {currentType === 'trainer' && (
                            <div>
                                <label className="text-xs font-bold text-[color:var(--mk-muted)] uppercase tracking-wider block mb-2">Mức giá / tháng</label>
                                <div className="flex flex-wrap gap-2">
                ...
                {isLoading ? (
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 animate-pulse">
                        {[1, 2, 3, 4, 5, 6].map(i => (
                            <div key={i} className="card border border-[color:var(--mk-line)] flex flex-col">
                                <div className="flex items-center gap-4 mb-4">
                                    <div className="w-16 h-16 rounded-xs bg-[color:var(--mk-paper-strong)] shrink-0"></div>
                                    <div className="w-full space-y-2">
                                        <div className="h-5 bg-[color:var(--mk-paper-strong)] rounded-sm w-3/4"></div>
                                        <div className="h-4 bg-[color:var(--mk-paper-strong)] rounded-sm w-1/2"></div>
                                    </div>
                                </div>
                                <div className="space-y-2 mb-4 flex-1">
                                    <div className="h-3 bg-[color:var(--mk-paper-strong)] rounded-sm w-full"></div>
                                    <div className="h-3 bg-[color:var(--mk-paper-strong)] rounded-sm w-5/6"></div>
                                    <div className="h-3 bg-[color:var(--mk-paper-strong)] rounded-sm w-4/6"></div>
                                </div>
                                <div className="flex gap-2 mb-4">
                                    <div className="h-5 w-16 bg-[color:var(--mk-paper-strong)] rounded-sm"></div>
                                    <div className="h-5 w-20 bg-[color:var(--mk-paper-strong)] rounded-sm"></div>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="empty-state text-sm text-[color:var(--mk-muted)]">
                        <div className="empty-state-number">0</div>
                        <p className="text-sm font-medium text-[color:var(--mk-text-soft)]">Chưa có dữ liệu phù hợp.</p>
                        {hasActiveFilters && (
                    ...
                    <>
                        ...
                        {totalPages > 1 && (
                            <div className="mt-10 flex items-center justify-center gap-3">
                                ...
                                <span className="text-sm tabular-nums text-[color:var(--mk-text-soft)] min-w-[80px] text-center">
                                    {page} / {totalPages}
                                </span>
                                <button
