        ...
        <section className="py-12 sm:py-16">
            <div className="max-w-6xl mx-auto px-4 sm:px-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    {gymLinks.length > 0 && (
                        <div>
                            <h3 className="text-sm font-bold uppercase tracking-wider text-[color:var(--mk-muted)] mb-4">Phòng gym liên kết</h3>
                                ...
                                {gymLinks.map(link => {
                                    ...
                                    return (
                                        <Link
                                            key={link.id}
                                            to={`/gyms/${gymId}`}
                                            className="flex items-center gap-4 p-4 border border-[color:var(--mk-line)] rounded-lg hover:border-black transition-colors group"
                                        >
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                {link.branch?.branch_name && (
                                                    <div className="text-xs text-[color:var(--mk-muted)]">{link.branch.branch_name}</div>
                                                )}
                                            </div>
                                            {link.role_at_gym && (
                                                <span className="text-[10px] font-bold uppercase tracking-wider text-[color:var(--mk-muted)] shrink-0">
                                                    {link.role_at_gym}
                                                </span>
                                            )}
                                        </Link>
                    ...
                    <div className="space-y-6">
                        {mediaFeatures.length > 0 && (
                            <div>
                                <h3 className="text-sm font-bold uppercase tracking-wider text-[color:var(--mk-muted)] mb-4">Nội dung nổi bật</h3>
                                <div className="grid grid-cols-2 gap-3">
                                    {mediaFeatures.slice(0, 4).map(media => (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noreferrer"
                                            className="block rounded-lg overflow-hidden border border-[color:var(--mk-line)] hover:border-black transition-colors"
                                        >
                                            <img
                                            ...
                                            />
                                            {media.caption && (
                                                <div className="px-3 py-2 text-xs text-[color:var(--mk-text-soft)] truncate">{media.caption}</div>
                                            )}
                                        </a>
                                    ))}
                                </div>
                        ...
                        {pressMentions.length > 0 && (
                            <div>
                                <h3 className="text-sm font-bold uppercase tracking-wider text-[color:var(--mk-muted)] mb-4">Báo chí nhắc đến</h3>
                                <div className="space-y-2">
                                    {pressMentions.map(press => (
                                        <a
                                            ...
                                            className="flex items-start gap-3 p-3 rounded-lg hover:bg-[color:var(--mk-paper)] transition-colors"
                                        >
                                            <div className="w-8 h-8 bg-[color:var(--mk-paper)] rounded flex items-center justify-center text-[10px] font-bold text-[color:var(--mk-muted)] shrink-0">
                                                📰
                                            </div>
                                            <div className="min-w-0">
                                                <div className="text-[10px] font-bold uppercase tracking-wider text-[color:var(--mk-muted)]">{press.source_name}</div>
