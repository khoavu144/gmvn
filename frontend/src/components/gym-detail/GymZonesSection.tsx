            ...
            <section ref={setRef('zones')} id="zones" className="gym-detail-section marketplace-panel gv-panel-pad">
                ...
                <div className="grid gap-3.5 md:grid-cols-2 xl:grid-cols-3">
                    {branchZones.map((zone) => (
                        <article
                            key={zone.id}
                            className={`rounded-lg border px-4 py-4 ${zone.is_signature_zone ? 'border-amber-500/50 bg-amber-50/70' : 'border-gray-200/90 bg-white/75'}`}
                        >
                            ...
                            {zone.description && (
                                <p className="mt-2.5 text-sm leading-6 text-gray-600">{zone.description}</p>
                            )}
