    ...
    return (
        <div className="space-y-6">
                            ...
                            <div key={p.id} className="relative group border border-gray-200 rounded-xs overflow-hidden aspect-[3/4] bg-gray-50">
                                ...
                                <div className="absolute inset-0 bg-white/90 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-3 border border-black m-1">
                                    <div>
                                        ...
                                        {p.weight_kg && <p className="text-sm text-gray-900 mt-1 font-medium">{p.weight_kg} kg</p>}
                                        {p.caption && <p className="text-sm text-gray-600 mt-1 line-clamp-3">{p.caption}</p>}
                                    </div>
                                    <button
                                        onClick={() => {
                                            setConfirmConfig({ isOpen: true, photoId: p.id });
                                        }}
            ...
            <ConfirmModal
                ...
                isDestructive={true}
                onConfirm={() => {
                    if (confirmConfig.photoId) {
                        deleteMutation.mutate(confirmConfig.photoId);
                    }
