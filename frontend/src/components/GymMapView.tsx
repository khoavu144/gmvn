...
export default function GymMapView({ gyms }: Props) {
    ...
    if (pins.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-[520px] bg-gray-50 rounded-lg border border-dashed border-gray-200">
                <span className="text-5xl mb-4">🗺️</span>
                <p className="text-gray-500 font-medium">Chưa có cơ sở nào có tọa độ bản đồ.</p>
                <p className="text-xs text-gray-500 mt-1">Gym owner cần cập nhật latitude/longitude trong cài đặt chi nhánh.</p>
            </div>
        );
    }
