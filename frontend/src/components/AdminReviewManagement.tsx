        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                {filteredReviews.map(review => (
                    <div key={review.id} className={`bg-white border p-5 rounded-xl flex justify-between items-start transition-colors ${review.is_visible ? 'border-gray-200 hover:border-black' : 'border-dashed border-gray-300 opacity-60'}`}>
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                                <span className="font-black text-sm uppercase tracking-tight">Hội viên {review.user_id.substring(0, 8)}...</span>
                                <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest bg-gray-100 px-2 py-1 rounded-sm">
                ...
                {filteredReviews.length === 0 && (
                    <div className="text-gray-500 italic py-12 text-center border-2 border-dashed border-gray-100 rounded-xl">Không tìm thấy đánh giá nào phù hợp.</div>
                )}
            </div>
        </div>
    );
